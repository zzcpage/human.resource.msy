...
public interface LectureDao {
     ...
     * @return  返回教师的所有授课记录
     */
    @Select("select * from lecture where tid=#{tid}")
    public List<Lecture> findLectureByTid(@Param("tid") long tid);
     ...
     * @return     返回对应的授课记录
     */
    @Select("select * from lecture where cid=#{cid}")
    public Lecture findLectureByCid(@Param("cid") long cid);
     ...
     * @return  返回增加的结果
     */
    @Insert("insert into lecture(tid,cid) values(#{tid},#{cid})")
     ...
     */
    @Delete("delete from lecture where cid=#{cid}")
     ...
     */
    @Update("update lecture set tid=#{tid} , cid=#{cid} where id=#{id}")
